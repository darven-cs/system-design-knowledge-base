...
缓存可以位于客户端（操作系统或者浏览器），[服务端](/topics/reverse-proxy)或者不同的缓存层。

## CDN 缓存

[CDN](/topics/cdn) 也被视为一种缓存。

## Web 服务器缓存

[反向代理](/topics/reverse-proxy)和缓存（比如 [Varnish](https://www.varnish-cache.org/)）可以直接提供静态和动态内容。Web 服务器同样也可以缓存请求，返回相应结果而不必连接应用服务器。

...
We could store info on the 10 million users in a [relational database](/en/topics/database#relational-database-management-system-rdbms).  We should discuss the [use cases and tradeoffs between choosing SQL or NoSQL](/en/topics/database#sql-or-nosql).

* The **Client** sends a request to the **Web Server**, running as a [reverse proxy](/en/topics/reverse-proxy)
...
We'll create an [index](/en/topics/database#use-good-indices) on `id`, `user_id `, and `created_at` to speed up lookups (log-time instead of scanning the entire table) and to keep the data in memory.  Reading 1 MB sequentially from memory takes about 250 microseconds, while reading from SSD takes 4x and from disk takes 80x longer.<sup><a href="/en/topics/appendix#latency-numbers-every-programmer-should-know">1</a></sup>

We'll use a public [**REST API**](/en/topics/communication#representational-state-transfer-rest):
...
For internal communications, we could use [Remote Procedure Calls](/en/topics/communication#remote-procedure-call-rpc).
...
* The **Accounts API** server places a job on a **Queue** such as [Amazon SQS](https://aws.amazon.com/sqs/) or [RabbitMQ](https://www.rabbitmq.com/)
    * Extracting transactions could take awhile, we'd probably want to do this [asynchronously with a queue](/en/topics/asynchronism), although this introduces additional complexity
...
We'll create an [index](/en/topics/database#use-good-indices) on `id`, `user_id `, and `created_at`.
...
We'll create an [index](/en/topics/database#use-good-indices) on `id` and `user_id `.
...
State you would 1) **Benchmark/Load Test**, 2) **Profile** for bottlenecks 3) address bottlenecks while evaluating alternatives and trade-offs, and 4) repeat.  See [Design a system that scales to millions of users on AWS](/en/interview/solutions/scale-from-zero-to-millions) as a sample on how to iteratively scale the initial design.

It's important to discuss what bottlenecks you might encounter with the initial design and how you might address each of them.  For example, what issues are addressed by adding a **Load Balancer** with multiple **Web Servers**?  **CDN**?  **Master-Slave Replicas**?  What are the alternatives and **Trade-Offs** for each?

We'll introduce some components to complete the design and to address scalability issues.  Internal load balancers are not shown to reduce clutter.

*To avoid repeating discussions*, refer to the following [system design topics](/en/topics/) for main talking points, tradeoffs, and alternatives:

* [DNS](/en/topics/dns)
* [CDN](/en/topics/cdn)
* [Load balancer](/en/topics/load-balancer)
* [Horizontal scaling](/en/topics/load-balancer#horizontal-scaling)
* [Web server (reverse proxy)](/en/topics/reverse-proxy)
* [API server (application layer)](/en/topics/application-layer)
* [Cache](/en/topics/cache)
* [Relational database management system (RDBMS)](/en/topics/database#relational-database-management-system-rdbms)
* [SQL write master-slave failover](/en/topics/availability-patterns#fail-over)
* [Master-slave replication](/en/topics/database#master-slave-replication)
* [Asynchronism](/en/topics/asynchronism)
* [Consistency patterns](/en/topics/consistency-patterns)
* [Availability patterns](/en/topics/availability-patterns)
...
Refer to [When to update the cache](/en/topics/cache#when-to-update-the-cache) for tradeoffs and alternatives.  The approach above describes [cache-aside](/en/topics/cache#cache-aside).

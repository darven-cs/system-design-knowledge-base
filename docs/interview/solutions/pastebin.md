...
**注意: 为了避免重复，当前文档会直接链接到[系统设计主题](/topics/)的相关区域，请参考链接内容以获得综合的讨论点、权衡和替代方案。**
...
我们可以用一个 [关系型数据库](/topics/database#关系型数据库管理系统rdbms)作为一个大的哈希表，用来把生成的 url 映射到一个包含 paste 文件的文件服务器和路径上。

为了避免托管一个文件服务器，我们可以用一个托管的**对象存储**，比如 Amazon 的 S3 或者[NoSQL 文档类型存储](/topics/database#文档类型存储)。

作为一个大的哈希表的关系型数据库的替代方案，我们可以用[NoSQL 键值存储](/topics/database#键-值存储)。我们需要讨论[选择 SQL 或 NoSQL 之间的权衡](/topics/database#sql-还是-nosql)。下面的讨论是使用关系型数据库方法。

* **客户端** 发送一个创建 paste 的请求到作为一个[反向代理](/topics/reverse-proxy)启动的 **Web 服务器**。
...
我们将在 `shortlink` 字段和 `created_at` 字段上创建一个[数据库索引](/topics/database#使用正确的索引)，用来提高查询的速度（避免因为扫描全表导致的长时间查询）并将数据保存在内存中，从内存里面顺序读取 1MB 的数据需要大概 250 微秒，而从 SSD 上读取则需要花费 4 倍的时间，从硬盘上则需要花费 80 倍的时间。<sup><a href="/topics/appendix#每个程序员都应该知道的延迟数" > 1</a></sup>
...
我们将会用一个公开的 [**REST 风格接口**](/topics/communication#表述性状态转移rest)：
...
用于内部通信，我们可以用 [RPC](/topics/communication#远程过程调用协议rpc)。
...
说明您将迭代地执行这样的操作：1)**Benchmark/Load 测试**，2)**Profile** 出瓶颈，3)在评估替代方案和权衡时解决瓶颈，4)重复前面，可以参考[在 AWS 上设计一个可以支持百万用户的系统](/interview/solutions/scale-from-zero-to-millions)这个用来解决如何迭代地扩展初始设计的例子。
...
**为了避免重复的讨论**， 参考以下[系统设计主题](/topics/)获取主要讨论要点、权衡和替代方案：

* [DNS](/topics/dns)
* [CDN](/topics/cdn)
* [负载均衡器](/topics/load-balancer)
* [水平扩展](/topics/load-balancer#水平扩展)
* [反向代理（web 服务器）](/topics/reverse-proxy)
* [应用层](/topics/application-layer)
* [缓存](/topics/cache)
* [关系型数据库管理系统 (RDBMS)](/topics/database#关系型数据库管理系统rdbms)
* [SQL write master-slave failover](/topics/availability-patterns#故障切换)
* [主从复制](/topics/database#主从复制)
* [一致性模式](/topics/consistency-patterns)
* [可用性模式](/topics/availability-patterns)
...
* [联合](/topics/database#联合)
* [分片](/topics/database#分片)
* [非规范化](/topics/database#非规范化)
...
* [键值存储](/topics/database#键-值存储)
* [文档存储](/topics/database#文档类型存储)
* [列型存储](/topics/database#列型存储)
* [图数据库](/topics/database#图数据库)
* [sql 还是 nosql](/topics/database#sql-还是-nosql)

### 缓存

* 在哪缓存
  * [客户端缓存](/topics/cache#客户端缓存)
  * [CDN 缓存](/topics/cache#cdn-缓存)
  * [Web 服务器缓存](/topics/cache#web-服务器缓存)
  * [数据库缓存](/topics/cache#数据库缓存)
  * [应用缓存](/topics/cache#应用缓存)
* 缓存什么
  * [数据库查询级别的缓存](/topics/cache#数据库查询级别的缓存)
  * [对象级别的缓存](/topics/cache#对象级别的缓存)
* 何时更新缓存
  * [缓存模式](/topics/cache#缓存模式)
  * [直写模式](/topics/cache#直写模式)
  * [回写模式](/topics/cache#回写模式)
  * [刷新](/topics/cache#刷新)

### 异步和微服务

* [消息队列](/topics/asynchronism#消息队列)
* [任务队列](/topics/asynchronism#任务队列)
* [背压](/topics/asynchronism#背压)
* [微服务](/topics/application-layer#微服务)

### 通信

* 讨论权衡:
  * 跟客户端之间的外部通信 - [HTTP APIs following REST](/topics/communication#表述性状态转移rest)
  * 内部通信 - [RPC](/topics/communication#远程过程调用协议rpc)
* [服务发现](/topics/application-layer#服务发现)

### 安全

参考[安全](/topics/security)。

### 延迟数字

见[每个程序员都应该知道的延迟数](/topics/appendix#每个程序员都应该知道的延迟数)。
